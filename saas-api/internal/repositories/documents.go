...
func (r *DocumentRepository) Create(ctx context.Context, doc *Document) error {
	...
	query := `
		INSERT INTO documents (id, org_id, folder_id, parent_id, doc_id, name, file_path, json_file_path, status, content, metadata, created_by, created_at, uploaded_at, updated_at)
...
// GetByID retrieves a document by its UUID ID
func (r *DocumentRepository) GetByID(ctx context.Context, id uuid.UUID) (*Document, error) {
	query := `
		SELECT d.id, d.org_id, d.folder_id, d.parent_id, d.doc_id, d.name, d.file_path, d.json_file_path, 
		...
		LEFT JOIN documents f ON d.folder_id = f.id
...
func (r *DocumentRepository) ListByFolder(ctx context.Context, folderID *uuid.UUID, orgID uuid.UUID, page, limit int) ([]*Document, int64, error) {
	...
	// Count query - filter by folder_id and org_id, exclude folders and deleted
	countQuery := `
		SELECT COUNT(*) 
		FROM documents 
		WHERE org_id = $1
	...
	// List query - filter by folder_id and org_id
	query := `
		SELECT d.id, d.org_id, d.folder_id, d.parent_id, d.doc_id, d.name, d.file_path, d.json_file_path,
		       d.status, d.content, d.metadata, d.created_by, d.created_at, d.uploaded_at, d.processed_at,
		       d.updated_by, d.updated_at, d.deleted_by, d.deleted_at,
...
// Delete removes a document by UUID ID (hard delete)
...
func (r *DocumentRepository) Update(ctx context.Context, doc *Document) error {
	...
	query := `
		UPDATE documents
		SET name = $1, folder_id = $2, file_path = $3, content = $4, metadata = $5, updated_by = $6, updated_at = NOW()
